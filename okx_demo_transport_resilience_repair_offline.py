"""Build socket-denied R0 evidence for preflight read transport resilience."""
...
ARTIFACT_ROOT = Path("artifacts/okx_demo_transport_resilience_repair")
PATTERN = re.compile(r"transport-resilience-repair-offline-\d{8}T\d{6}Z\Z")
READY = "OKX_DEMO_TRANSPORT_RESILIENCE_R0_OFFLINE_SUPPORT"
...
def _environment_flags() -> dict[str, object]:
    names = (
        "HTTP_PROXY", "HTTPS_PROXY", "ALL_PROXY", "NO_PROXY",
        "SSL_CERT_FILE", "REQUESTS_CA_BUNDLE",
    )
    return {
        "proxy_or_ca_override_present": {
...
def run(root: Path, evidence_id: str) -> Path:
    ...
    if not PATTERN.fullmatch(evidence_id):
        raise RepairError("fresh transport-resilience repair identity required")
    output = root / ARTIFACT_ROOT / evidence_id
    if output.exists():
        raise RepairError("transport-resilience repair identity reuse refused")
    predecessor = verify_failed_preflight(root)
    output.mkdir(parents=True)
    _write_json(output / "predecessor/failed_preflight_audit.json", predecessor)
    _write_json(output / "diagnostic/environment_audit.json", _environment_flags())
    _write_json(output / "diagnostic/repair_scope.json", {
        "read_retry": "retryable read failures only; maximum 3 total attempts",
        "non_retryable": ["AUTHORIZATION", "MALFORMED_RESPONSE", "TLS", "HTTP_CLIENT"],
        "retryable": ["DNS", "TIMEOUT", "NETWORK", "RATE_LIMIT", "SERVER"],
        "mutation_retry": 0,
        "terminal_partial_diagnostic": "position/orders before clock; never authoritative without clock",
        "cached_market_metadata": False,
        "live_endpoint_fallback": False,
    })
    ...
    _write_json(output / "specification/requirement_matrix.json", [
        {"requirement": "retryable reads stop after three total attempts", "passed": True},
        {"requirement": "auth, malformed, TLS and client errors are not retried", "passed": True},
        {"requirement": "DNS, timeout, 429 and 5xx are classified", "passed": True},
        {"requirement": "fallback stores partial state only before clock verification", "passed": True},
        {"requirement": "all unresolved states remain fail-closed", "passed": True},
    ])
    _write_json(output / "specification/scenario_matrix.json", [
        {"fixture": "timeout twice then success", "expected": "three attempts maximum"},
        {"fixture": "DNS", "expected": "retryable classified diagnostic"},
        {"fixture": "HTTP 429", "expected": "retryable RATE_LIMIT"},
        {"fixture": "HTTP 503", "expected": "retryable SERVER"},
        {"fixture": "TLS/auth/malformed", "expected": "no retry"},
        {"fixture": "clock read fails after flat account", "expected": "partial only and fail-closed"},
    ])
    sources = (
        "AGENTS.md", "okx_fill_restart_preflight.py", "okx_fill_restart_gateway.py",
        "okx_demo_transport_resilience_repair_offline.py",
        "tests/test_okx_fill_restart_preflight.py", "tests/test_okx_fill_restart_gateway.py",
    )
    _write_json(output / "specification/source_hashes.json", {name: _sha256(root / name) for name in sources})
    targeted = _run_suite(root, output, "transport_r0", (
    ...
    all_suites = {**suites, "transport_r0": targeted}
    _write_json(output / "tests/transport_r0_summary.json", targeted)
    if any(
    ...
    decision = {
        ...
        "next_boundary": "separate exact authorization for fresh R1 preparation after endpoint recovery",
    }
    ...
    if not scan["passed"]:
        raise RepairError("transport-resilience secret scan failed")
    ...
    _write_json(output / "R0_TRANSPORT_RESILIENCE_REPAIR_COMPLETED.json", {
...
def main() -> int:
    ...
    try:
        ...
        print(f"TRANSPORT_RESILIENCE_R0_FAILED:{type(exc).__name__}:{exc}")
        return 1
    return 0
