"""Freeze one future-book successor formal OKX Demo package offline."""
...
REPAIR_ID = "future-book-repair-offline-20260806T160215Z"
PREFLIGHT_RUN_ID = "preflight-20260807T125759Z"
FAILED_PACKAGE_ID = "formal-package-20260806T152904Z"
FAILED_FORMAL_RUN_ID = "formal-20260806T152904Z"
REPAIR_ARTIFACT_ROOT = Path("artifacts") / "okx_demo_future_book_timestamp_repair"
SUCCESSOR_SOURCE_FILES = (
    "okx_future_book_formal_prepare.py",
...
class FutureBookFormalPackageError(RuntimeError):
    pass
...
def _source_hashes(root: Path) -> dict[str, str]:
    ...
    for relative in SUCCESSOR_SOURCE_FILES:
        path = root / relative
        if not path.is_file():
            raise FutureBookFormalPackageError(
...
def _predecessor_template(root: Path) -> dict[str, object]:
    ...
    if not all(path.is_file() for path in (terminal_path, manifest_path, spec_path)):
        raise FutureBookFormalPackageError("failed predecessor is incomplete")
    ...
    if (
        ...
        or terminal.get("final_position_btc") != "0"
        or terminal.get("final_open_order_count") != 0
        or terminal.get("normal_bid_fills") != 0
        or terminal.get("normal_ask_fills") != 0
        or terminal.get("special_fill_count") != 0
    ):
        raise FutureBookFormalPackageError("failed predecessor terminal drift")
    ...
    if not isinstance(expected, str) or _sha256(spec_path) != expected:
        raise FutureBookFormalPackageError("failed predecessor template hash mismatch")
    template = _json(spec_path)
    if (
        template.get("package_id") != FAILED_PACKAGE_ID
        or template.get("formal_run_id") != FAILED_FORMAL_RUN_ID
    ):
        raise FutureBookFormalPackageError("failed predecessor identity drift")
    return template
...
def _repair_test_contract(root: Path, repair_id: str) -> dict[str, object]:
    ...
    if any(
        ...
        raise FutureBookFormalPackageError("carried future-book repair gate failed")
    return {
        "repair_test_summary_sha256": _sha256(full_path),
        "future_book_targeted_passed": summary["future_book_targeted"]["passed"],
...
def _build_spec(
    *,
    ...
    if not package_id.startswith("formal-package-") or any(
        ...
        raise FutureBookFormalPackageError("formal package ID is invalid")
    if not formal_run_id.startswith("formal-") or any(
        character.isspace() for character in formal_run_id
    ):
        raise FutureBookFormalPackageError("formal run ID is invalid")
    if package_id == FAILED_PACKAGE_ID or formal_run_id == FAILED_FORMAL_RUN_ID:
        raise FutureBookFormalPackageError("failed predecessor ID reuse refused")
    if (
        repair_audit.get("evidence_kind") != "future_book_timestamp_repair"
        ...
        raise FutureBookFormalPackageError(
            "future-book repair or preflight evidence is invalid"
        )
    ...
    if template.get("profile_binding_sha256") != profile.binding_sha256:
        raise FutureBookFormalPackageError("promoted profile binding drift")
    ...
    if risk_budget != expected_risk:
        raise FutureBookFormalPackageError("frozen risk budget drift")
    ...
    runtime["future_book_timestamp_contract"] = {
        "signed_age": "local_receive_ms - exchange_book_timestamp_ms",
        ...
        "invalid_or_out_of_budget": "fail_closed_before_mutation",
        "shared_preflight_formal_semantics": True,
    ...
    base: dict[str, object] = {
        ...
        "phase": "FUTURE_BOOK_SUCCESSOR_FORMAL_PACKAGE_FREEZE_OFFLINE",
        "package_id": package_id,
        "formal_run_id": formal_run_id,
        ...
        "fixture_manifest": {
            "carried_future_book_repair_gates": fixture_contract,
...
def _run_targeted_suite(root: Path, output: Path) -> dict[str, object]:
    ...
    audit_path = tests / "future_book_formal_targeted_network_audit.json"
    base_temp = Path(tempfile.mkdtemp(prefix="okx-future-book-formal-"))
    command = [
        sys.executable,
        "-m",
        "pytest",
    ...
    _write_text(tests / "future_book_formal_targeted.txt", completed.stdout)
...
def prepare_future_book_formal_package(
    root: Path,
    *,
    ...
    with guard:
        ...
        if (
            ...
            raise FutureBookFormalPackageError(
                "preflight is not bound to the selected future-book repair"
            )
        hashes = _source_hashes(root)
        package_id, formal_run_id = _fresh_identifiers()
        output = root / ARTIFACT_ROOT / package_id
        if output.exists():
            raise FutureBookFormalPackageError("formal package ID collision")
        if any(
            ...
            raise FutureBookFormalPackageError("formal run ID reuse refused")
        ...
        _write_json(output / "tests" / "test_summary.json", {
            "future_book_formal_targeted": targeted,
            "carried_future_book_repair_gates": spec["fixture_manifest"],
        })
        if not targeted["passed_gate"]:
            raise FutureBookFormalPackageError(
                "future-book successor regression failed"
            )
        ...
        decision = {
            ...
            "future_book_timestamp_repair_bound": True,
            "package_id": package_id,
            "formal_run_id": formal_run_id,
            "session_id": formal.session_id,
            "formal_execution_armed": False,
        ...
        _write_text(
            ...
            "# Future-book successor formal OKX Demo package\n\n"
...
def main() -> int:
    ...
    try:
        output, formal = prepare_future_book_formal_package(
            args.root,
            repair_id=args.repair_id,
            preflight_run_id=args.preflight_run_id,
        )
    except Exception as exc:
        print(f"FUTURE_BOOK_FORMAL_PACKAGE_FAILED:{type(exc).__name__}:{exc}")
        return 1
    print(json.dumps({
        "output": str(output),
