"""Freeze one R2-warmup/cumulative-audit successor formal package offline."""
...
REPAIR_ID = "r2-repair-offline-20260807T133755Z"
PREFLIGHT_RUN_ID = "preflight-20260807T133842Z"
FAILED_PACKAGE_ID = "formal-package-20260807T130847Z"
FAILED_FORMAL_RUN_ID = "formal-20260807T130847Z"
REPAIR_ARTIFACT_ROOT = Path("artifacts") / "okx_demo_r2_warmup_audit_repair"
SUCCESSOR_SOURCE_FILES = (
    "okx_r2_warmup_audit_formal_prepare.py",
...
class R2FormalPackageError(RuntimeError):
    pass
...
def _fresh_identifiers(now: datetime | None = None) -> tuple[str, str]:
    ...
    if observed.tzinfo is None:
        raise R2FormalPackageError("formal identifier timestamp must be timezone aware")
...
def _source_hashes(root: Path) -> dict[str, str]:
    ...
    for relative in SUCCESSOR_SOURCE_FILES:
        path = root / relative
        if not path.is_file():
            raise R2FormalPackageError(
                f"R2 successor formal source is missing: {relative}"
            )
...
def _predecessor_template(root: Path) -> dict[str, object]:
    ...
    kill_path = package / "audits" / "formal" / "kill_switch_audit.json"
    required = (terminal_path, manifest_path, spec_path, kill_path)
    if not all(path.is_file() for path in required):
        raise R2FormalPackageError("failed R2 predecessor is incomplete")
    terminal = _json(terminal_path)
    kill = _json(kill_path)
    if any((
        terminal.get("status") != "OKX_DEMO_FILL_RESTART_SAFETY_FAILED",
        terminal.get("formal_run_id") != FAILED_FORMAL_RUN_ID,
        terminal.get("formal_execution_marker_count") != 1,
        terminal.get("R1_completed") is not True,
        terminal.get("R2_completed") is not False,
        ...
        kill.get("kill_active_at_terminal") is not True,
        (package / "formal_run" / "restart" / "R2_resume.json").exists(),
    )):
        raise R2FormalPackageError("failed R2 predecessor terminal drift")
    ...
    if not isinstance(expected, str) or _sha256(spec_path) != expected:
        raise R2FormalPackageError("failed R2 predecessor template hash mismatch")
    template = _json(spec_path)
    if (
        template.get("package_id") != FAILED_PACKAGE_ID
        or template.get("formal_run_id") != FAILED_FORMAL_RUN_ID
    ):
        raise R2FormalPackageError("failed R2 predecessor identity drift")
    return template
...
def _repair_test_contract(root: Path, repair_id: str) -> dict[str, object]:
    ...
    required = (
        "r2_repair_targeted", "root_non_optuna", "backtest_non_optuna",
    )
    if any(
        ...
        raise R2FormalPackageError("carried R2 repair gate failed")
    return {
        "repair_test_summary_sha256": _sha256(full_path),
        "r2_repair_targeted_passed": summary["r2_repair_targeted"]["passed"],
...
def _build_spec(
    *,
    ...
    if not package_id.startswith("formal-package-") or any(
        ...
        raise R2FormalPackageError("formal package ID is invalid")
    if not formal_run_id.startswith("formal-") or any(
        character.isspace() for character in formal_run_id
    ):
        raise R2FormalPackageError("formal run ID is invalid")
    if package_id == FAILED_PACKAGE_ID or formal_run_id == FAILED_FORMAL_RUN_ID:
        raise R2FormalPackageError("failed formal predecessor ID reuse refused")
    if any((
        repair_audit.get("evidence_kind") != "r2_warmup_audit_repair",
        ...
        raise R2FormalPackageError("R2 repair or preflight evidence is invalid")
    ...
    if template.get("profile_binding_sha256") != profile.binding_sha256:
        raise R2FormalPackageError("promoted profile binding drift")
    ...
    if risk_budget != expected_risk:
        raise R2FormalPackageError("frozen risk budget drift")
    ...
    runtime["r2_warmup_audit_contract"] = {
        "new_process_quote_engine_warmed_before_probe": True,
        "warmup_uses_bounded_monotonic_book_sequence": True,
        "warmup_and_probe_protected_mutation_delta": 0,
        "persisted_kill_required_before_probe": True,
        "hash_chained_gateway_audit_per_generation": True,
        "terminal_counters_cumulative_across_generations": True,
        "generations_contiguous": True,
        "same_generation_counters_monotonic": True,
        "mutation_methods": ["cancel_order", "create_order"],
        "terminal_order_event_reconciliation": True,
        "invalid_or_unknown": "fail_closed_with_kill_latch_active",
    }
    ...
    base: dict[str, object] = {
        ...
        "phase": "R2_WARMUP_AUDIT_SUCCESSOR_FORMAL_PACKAGE_FREEZE_OFFLINE",
        "package_id": package_id,
        "formal_run_id": formal_run_id,
        ...
        "fixture_manifest": {
            "carried_r2_repair_gates": fixture_contract,
...
def _run_targeted_suite(root: Path, output: Path) -> dict[str, object]:
    ...
    audit_path = tests / "r2_formal_targeted_network_audit.json"
    base_temp = Path(tempfile.mkdtemp(prefix="okx-r2-formal-"))
    command = [
    ...
    _write_text(tests / "r2_formal_targeted.txt", completed.stdout)
    ...
    *,
    ...
    with guard:
        ...
        if any(
            ...
            raise R2FormalPackageError("formal run ID reuse refused")
        ...
        _write_json(output / "tests" / "test_summary.json", {
            "r2_formal_targeted": targeted,
            "carried_r2_repair_gates": spec["fixture_manifest"],
        })
        if not targeted["passed_gate"]:
            raise R2FormalPackageError("R2 successor formal regression failed")
        if _source_hashes(root) != hashes:
            raise R2FormalPackageError("formal source changed during package freeze")
        ...
        if not secret_scan["passed"]:
            raise R2FormalPackageError("secret pattern found in formal package")
        decision = {
            "status": FORMAL_PACKAGE_STATUS,
            "r2_warmup_audit_repair_bound": True,
            "package_id": package_id,
            "formal_run_id": formal_run_id,
            "session_id": formal.session_id,
            "formal_execution_armed": False,
        ...
        _write_text(
            ...
            "# R2-warmup successor formal OKX Demo package\n\n"
    ...
    if guard.attempts:
        raise R2FormalPackageError("network attempt blocked during package freeze")
    loaded = load_frozen_package(root, formal.package_id)
    if loaded.spec != formal:
        raise R2FormalPackageError("frozen package controller verification failed")
...
def main() -> int:
    ...
    try:
        output, formal = prepare_r2_formal_package(
            args.root,
            repair_id=args.repair_id,
            preflight_run_id=args.preflight_run_id,
        )
    except Exception as exc:
        print(f"R2_FORMAL_PACKAGE_FAILED:{type(exc).__name__}:{exc}")
        return 1
    print(json.dumps({
        "output": str(output),
